...
public class Banco implements Serializable {
    private static final String MENSAJE_ERROR = "Se ha producido un error al leer el archivo:";
    ...
    public void guardarEstado(String nombreArchivo) throws IOException {
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(nombreArchivo))) {
            ...
            throw new IOException(MENSAJE_ERROR + nombreArchivo);
        }
    ...
    private int buscarCuenta(String codigo) {
        ...
        for (int i = 0; i < numeroCuentas; i++) {
            if (cuentas[i].data.data.getIban().equals(codigo)) {
                return i;
            }
        }
        return -1;
    ...
    public StringBuilder informeCuentas() {
        ...
        for (int i=0;i<this.numeroCuentas;i++){
            salida.append(cuentas[i].toString());
            salida.append("\n");
            totalSaldo += cuentas[i].data.data.getSaldo();
            numCuentas++;
        }
